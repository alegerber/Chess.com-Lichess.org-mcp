...
└── tools/
    ├── chess.ts      # Registers the 28 Chess.com tools (+ their formatters)
    └── lichess.ts    # Registers the 26 Lichess tools (+ their formatters)
```
...
- **`src/tools/chess.ts` / `src/tools/lichess.ts`** register the 54 tools (28 Chess.com + 26 Lichess) via `server.registerTool()` and hold each tool's formatter. Handlers wrap their API call in a local `call(fn, format)` helper so failures are returned as tagged error results.
