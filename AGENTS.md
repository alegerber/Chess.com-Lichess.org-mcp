...
└── tools/
    ...
    └── lichess.ts    # Registers the 25 Lichess tools (+ their formatters)
```
...
- **`src/tools/chess.ts` / `src/tools/lichess.ts`** register the 53 tools (28 Chess.com + 25 Lichess) via `server.registerTool()` and hold each tool's formatter. Handlers wrap their API call in a local `call(fn, format)` helper so failures are returned as tagged error results.
