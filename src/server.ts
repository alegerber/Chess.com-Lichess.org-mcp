...
const INSTRUCTIONS = `Read-only access to the public Chess.com and Lichess APIs — player profiles, games, stats, puzzles, clubs, teams, tournaments, leaderboards, and more. No authentication is required for the public endpoints, and nothing is ever modified. An optional LICHESS_TOKEN environment variable unlocks OAuth-only reads (e.g. a user's teams). Chess.com tools use plain names (e.g. get_player_profile); Lichess tools are prefixed with "lichess_" (e.g. lichess_get_user). Chess.com timestamps are in seconds, Lichess in milliseconds.`;
